...
- **Windows**: `자막생성기.bat` 파일을 더블클릭하여 실행합니다.
  ...
  - **시작 메뉴에 고정하는 방법**:
    - **방법 1) 바로가기 만들어 '시작'에 고정**
      1. 배치파일(.bat)을 우클릭 → 바로가기 만들기를 선택합니다.
      2. 만들어진 "~바로가기"를 우클릭 → 속성 → '대상(T)'을 아래처럼 바꿉니다.
         ```
         cmd /c "배치파일전체경로.bat"
         ```
      3. (선택) 같은 속성 창에서 아이콘 변경도 할 수 있습니다.
      4. 해당 바로가기를 우클릭 → **시작 화면에 고정(또는 시작에 고정)**을 선택합니다.
    - **방법 2) 시작 메뉴 '프로그램' 목록에 넣기(폴더에 바로가기 복사)**
      - 시작 메뉴의 "모든 앱" 목록에 보이게 하려면, 배치파일 자체가 아니라 "바로가기"를 시작 메뉴 프로그램 폴더에 넣으면 됩니다.
      - 사용자별(현재 계정만):
        ```
        C:\Users\%username%\AppData\Roaming\Microsoft\Windows\Start Menu\Programs
        ```
      - 전체 사용자(모든 계정 공통)도 유사하게 Start Menu\Programs 경로를 사용합니다(환경에 따라 권한 필요).
      - 위 폴더에 배치파일의 "바로가기(.lnk)"를 넣으면 시작 메뉴에서 검색/실행이 쉬워집니다.
